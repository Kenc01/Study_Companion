...
function migrateTopic(t: Record<string, unknown>, idx: number): Topic | null {
  ...
  return {
    ...
    tags: Array.isArray(t.tags)
      ...
      : [],
    color:
      typeof t.color === "number" ? Math.max(0, Math.min(5, t.color)) : idx % 6,
    rawNotes: typeof t.rawNotes === "string" ? t.rawNotes : "",
    createdAt: Number(t.createdAt) || Date.now(),
    updatedAt: Number(t.updatedAt) || Date.now(),
    lastStudiedAt:
      typeof t.lastStudiedAt === "number" ? t.lastStudiedAt : undefined,
    questions,
  };
}
...
function sanitizeMastery(map: MasteryMap): MasteryMap {
  ...
  for (const [key, value] of Object.entries(map)) {
    ...
    out[key] = {
      streak: Number.isFinite(rec.streak)
        ? Math.max(0, Math.trunc(rec.streak))
        : 0,
      mastered: Boolean(rec.mastered),
      correctCount: Number.isFinite(rec.correctCount)
        ? Math.max(0, rec.correctCount)
        : 0,
      ...
      srs: rec.srs
        ? {
            ...
            interval: Number.isFinite(rec.srs.interval)
              ? Math.max(0, rec.srs.interval)
              : 0,
            dueAt: Number.isFinite(rec.srs.dueAt) ? rec.srs.dueAt : Date.now(),
            lastQuality:
              rec.srs.lastQuality !== undefined &&
              Number.isFinite(rec.srs.lastQuality)
...
function sanitizeSettings(s: unknown): Settings {
  ...
  return {
    ...
    autoAdvanceMs:
      typeof rec.autoAdvanceMs === "number" &&
      Number.isFinite(rec.autoAdvanceMs)
...
function sanitizeQuestion(q: Record<string, unknown>): Question | null {
  ...
  return {
    ...
    options: Array.isArray(q.options)
      ? (q.options as string[]).filter(
          (o): o is string => typeof o === "string",
        )
      : undefined,
    answerIndices: Array.isArray(q.answerIndices)
      ? (q.answerIndices as number[]).filter(
    ...
    tags: Array.isArray(q.tags)
      ? (q.tags as string[]).filter((t): t is string => typeof t === "string")
      : [],
