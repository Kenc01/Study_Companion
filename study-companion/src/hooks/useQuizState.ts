...
import { gradeAnswer, gradeMcqAnswer, gradeMultiAnswer } from '@/lib/matchAnswer'
...
export interface UseQuizState {
  ...
  updateQuestionBookmark: (topicId: string, questionId: string, bookmarked: boolean) => void
  ...
  importData: (data: StudyCompanionExport, merge?: boolean) => { ok: boolean; message: string }
  resetAllData: () => void
...
export function useQuizState(): UseQuizState {
  ...
  const [lastSession, setLastSession] = React.useState<QuizSession | null>(null)
  ...
  const saveTopic = React.useCallback<UseQuizState['saveTopic']>(
    ({ name, rawNotes, questions, tags, color }) => {
      ...
      if (editingId) {
        ...
        if (existing) {
          const byPrompt = new Map(existing.questions.map((q) => [q.prompt.toLowerCase(), q.id]))
      ...
      setTopics((prev) =>
        prev.map((t) =>
          t.id === topicId
            ? {
                ...t,
                questions: t.questions.map((q) => (q.id === questionId ? { ...q, bookmarked } : q)),
              }
            : t,
        ),
      )
  ...
  const updateTopicTags = React.useCallback((topicId: string, tags: string[]) => {
    setTopics((prev) => prev.map((t) => (t.id === topicId ? { ...t, tags } : t)))
  }, [])
  ...
  const beginSession = React.useCallback(
    (topic: Topic, questions: Question[], mode: QuizMode) => {
      ...
      setScreen('quiz')
      setTopics((prev) =>
        prev.map((t) => (t.id === topic.id ? { ...t, lastStudiedAt: Date.now() } : t)),
      )
    },
    [],
  )
  ...
  const selectQuestionsForMode = React.useCallback(
    (topic: Topic, mode: QuizMode): Question[] => {
      switch (mode) {
        ...
        case 'review': {
          const due = topic.questions.filter((q) => isDue(mastery[masteryKey(topic.id, q.id)]))
  ...
  const startQuizForTopic = React.useCallback(
    (topic: Topic, mode: QuizMode = 'all') => beginSession(topic, topic.questions, mode),
    [beginSession],
  )
  ...
  const retryWrong = React.useCallback(() => {
    ...
    const ids = lastSession.logs.filter((l) => !l.correct).map((l) => l.questionId)
  ...
  const submitAnswer = React.useCallback<UseQuizState['submitAnswer']>(
    (value) => {
      ...
      if (question.type === 'mcq' && question.answerIndices && question.options) {
        const selected = Array.isArray(value) ? (value as number[]) : []
        correct = gradeMcqAnswer(selected, question.answerIndices)
        displayAnswer = question.answerIndices.map((i) => question.options![i]).join('; ')
        ...
        const answers = Array.isArray(value) ? (value as string[]) : typeof value === 'string' ? [value] : ['']
      ...
      const quality = qualityFromResult({ correct, hintsUsed, firstTry: true, fastAnswer })

      const key = masteryKey(session.topicId, question.id)
      const prev = mastery[key] ?? EMPTY_RECORD
      const streak = correct ? prev.streak + 1 : 0
      const mastered = correct ? prev.mastered || streak >= MASTERY_THRESHOLD : prev.mastered
    ...
    setSession((s) => {
      ...
      if (s.mode === 'cram') {
        ...
        if (!nextPending.length) {
          const finished = { ...s, pendingIds: [], clearedCount: cleared, awaitingNext: false }
      ...
      return { ...s, index: s.index + 1, awaitingNext: false, step: s.step + 1, hintLevel: 0 }
    })
  }, [])
  ...
  const importData = React.useCallback(
    (data: StudyCompanionExport, merge = false): { ok: boolean; message: string } => {
      if (!data || typeof data !== 'object') return { ok: false, message: 'Invalid file.' }
      if (data.version !== 2 && data.version !== 1) {
        return { ok: false, message: `Unsupported version: ${data.version}.` }
      }
      const incomingTopics = Array.isArray(data.topics) ? (data.topics as Topic[]) : []
      const incomingMastery = data.mastery && typeof data.mastery === 'object' ? (data.mastery as MasteryMap) : {}
      ...
      if (merge) {
        ...
        const remapped: Topic[] = incomingTopics.map((t) => {
          ...
          return { ...t, id: newId, questions: t.questions.map((q) => ({ ...q, id: uid('q') })) }
        })
      ...
      setSettings(incomingSettings)
      return { ok: true, message: `Replaced library with ${incomingTopics.length} topic(s).` }
    },
    [],
  )
